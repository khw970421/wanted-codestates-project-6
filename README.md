...
## 🚀 배포 링크
https://caredoc-khw970421.netlify.app/
...
- [x] 하나를 선택했다면 다음 화면으로 넘어갈 수 있습니다.  => clicked.allTime || clicked.partTime 중 하나라도 true라면 다음 화면 넘어갈 Button이 동작하게 구현

#### 스케쥴 입력 (Schedule.jsx)
- [x] 시작날짜, 종료날짜, 돌봄 시작 시간, 하루 돌봄 시간을 각각 입력받습니다.
- [x] 시작날짜, 종료날짜는 날짜 택스트 YYYY-MM-DD형식으로 전달합니다. => ConvertDateToStr 함수를 통해 매개변수로 받은 new Date를 YYYY-MM-DD 형식으로 리턴
...
- [x] 시작날짜, 종료날짜, 돌봄 시작 시간, 하루 돌봄 시간를 모두 입력하면 다음 버튼이 활성화  => checkButtonAvailable 함수를 통해 4가지 조건에 대해 true일때 버튼 활성화

#### 주소 입력 (Address.jsx)
- [x] 주소를 검색하는 기능은 juso.go.kr에서 제공하고 있는 OPEN API를 이용하여 구현
- [x] juso Open API에서 검색되어 선택된 모든 정보를 다 API의 address정보를 채우시는 데 활용 => 모달창으로 주소를 페이지네이션을 통해 추가 검색 가능
- [x] 추가로 addressDetail 부분에 상세 주소를 입력받아 주세요.
- [x] 주소정보, 상세주소를 입력하면 다음 버튼이 활성화 됩니다. => checkButtonAble 함수를 통해 2가지 조건 체크 후 버튼 활성화
